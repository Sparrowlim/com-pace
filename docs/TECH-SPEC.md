...
- `findByDate`/`findByDateRange`는 `date` 인덱스가 있는 스토어(`tasks`/`blocks`/`energyCells`/`sessions`)에서만 동작 — `predictions`는 `blockId`로만 키잉되고 날짜가 없어 제외(§10 로깅에서 필요하면 `blocks.date`를 거쳐 조인).
...
```
Task        { id, title, date, createdAt, splitDone }
Block       { id, taskId, verbLabel, status, date, startedAt, endedAt }
...
- `Task.date`/`Block.date`는 `createdAt`/`startedAt`의 날짜 버킷(YYYY-MM-DD, `energyCells`/`sessions`와 동일 패턴) — §10 내부 지표(시동 성공률·예측 적중률 시계열)가 날짜 범위로 훑을 수 있도록 v2(DB_VERSION)에서 인덱스 추가.
...
- [ ] Style Dictionary 빌드 스크립트 도입 시점(컴포넌트 토큰 단계, DESIGN-TOKENS §10-5) → 구현은 [`phases/PH-04-tokens-ui.md`](phases/PH-04-tokens-ui.md)에서 다룬다
- [x] `idb` 스키마 마이그레이션 전략(버전업 시) → §10 로깅 대비 `date` 인덱스 추가(v2)로 확정: `upgrade()`에서 `oldVersion` 분기 + 기존 레코드는 커서로 순회해 `date`를 `createdAt`/`startedAt`에서 백필(`idb-storage.ts`)
...
- **v0.2** — §12 미결 사항에 `phases/PH-01`·`PH-04` 교차 링크 추가(구현 위상 계획과 싱크).
- **v0.3** — §10 로깅 대비 DB 개편: `Task`/`Block`에 `date` 필드·인덱스 추가(`DB_VERSION` 1→2), `Storage.findByDateRange` 추가, `upgrade()` 마이그레이션에 `oldVersion` 분기 + 기존 레코드 `date` 백필 도입. §12 미결 항목 해소.
