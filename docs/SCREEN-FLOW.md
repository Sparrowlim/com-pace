# 컴페이스 — 화면 흐름 명세 (Screen Flow Spec) v0.2
...
| **2z** | 빈 대시보드(zero) | B 실행 | **2회차+ 과제 소진** 상태, "+새 과제" 저마찰 진입 | NEXT(다음 블록 없음) · 재진입(과제 없음) | – | 신규(와이어 필요) |
| **3-A** | 새 과제 추가 | B 실행 | 과제 소진 후 다음 과제 입력(One Task 유지) | 2z | – | 재구성 |
| **4** | 사전 예측 | B 실행 | "이번 15분에 끝날까?" 2지선다 | 3 · 2(새 블록) · NEXT(새 블록) | `끝날 것 같다` / `더 걸릴 듯` | 유지 |
| **5** | 집중 화면 | B 실행 | 15분 타이머 실행 | 4 · 이어하기(예측 생략 직행) | `진행` / `일시정지(5-B)` / `딴생각모달(5-A)` | 유지 |
| **5-A** | 딴생각 포착 | C 이탈 | 스친 생각 1줄 저장(타이머 **안 멈춤**), 종료 후 1회성 표시 | 5 (탭) | 모달 | 유지·모달 |
...
flowchart TD
    ...
    FOCUS -->|"길게 누름 / 앱 이탈·백그라운드·강제종료"| PAUSE["5-B 일시정지<br/>(포그라운드에서만 흐름)"]
    PAUSE -->|"재개(복귀 시 이어짐)"| FOCUS
    PAUSE -->|"그만하기"| INC{"미완료 회고"}

    %% ===== D 회고 4조합 (SPEC §3, P12) =====
    FOCUS -->|"15분 종료 + 에너지 즉시 점등"| RETRO{"회고 판정<br/>완료? × 예측적중?"}
...
| 진행 | **앱 이탈·백그라운드·강제종료** | **5-B 일시정지** | SPEC §6: 포그라운드에서만 흐름, 복귀 시 이어짐 |
| 일시정지(5-B) | 재개 / 재오픈 복귀 | 진행 (이어짐) | SPEC §6 세션 복구 |
...
> 🟢 = `SPEC.md`/`DECISIONS.md`로 해소. v0.2 기준 **열린 논리 이슈 없음.**
...
| **P4** | 미완료 "여기까지" 목적지 | 🟢 SPEC §6 (대시보드(2), 이월 블록 다음날 그대로) |
| **P5** | 방전 "평소 모드로" 목적지 | 🟢 SPEC §5 (대시보드 2) |
| **P6** | 방전 대시보드 경로·회고 | 🟢 SPEC §5 (회고 전체 스킵, 예측 생략 §5주) |
| **P7** | 세션 복구(강제종료·백그라운드) | 🟢 SPEC §6 (일시정지, 복귀 시 이어짐) |
...
1. **와이어 재작업**(SPEC §13): 면죄부 3화면 복원 · 온보딩 첫 질문 교체 · **빈 대시보드(zero) 신규** · 에너지 바 자라는 형태 · 방전 상시 링크 · 설정 양가 목표 · 기록 페이지 삭제.
2. **밤/아침 pull 접점 설계**(SPEC R3 / DECISIONS 부록C) — push 없이 자연 조우.
3. 확정 시 와이어프레임 캡션을 이 문서 전이에 역동기화.
