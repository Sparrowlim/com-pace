...
- **v0.7(2026-07-18)** — **문서 정합성 감사 반영(코드 diff 없음).** v0.6이 "구현은 별도 위상에서 착수 예정"으로 남겨둔 5-C(조각 마무리 선택)가 같은 날 `PH-12`(`PH-12-fragment-wrapup.md` v0.2, 커밋 `3df37dc`)로 구현·Runnable State 통과까지 완료됐음을 확인 — `phases/README.md §1` PH-12 행 드리프트와 동일 계열로 발견, 정정. P15(이탈 안내 문구)는 `PH-12`에서도 범위 밖으로 유지(Positive Non-Goal), P16(딴생각 포착 용도 확장)도 별도 patch 대기 상태 그대로.
