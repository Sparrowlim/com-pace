...
- **(2026-07-08 SCREEN-FLOW 전이 소급 감사) LOW 갭 1건, 미해결·저위험:** SCREEN-FLOW §2 Mermaid의 실선 `DASH → SPLIT(과제 재편집)` 전이에 대응하는 진입점이 대시보드에 없다(`DashboardPage`의 어떤 분기도 `/split` 재진입을 노출 안 함 — 쪼개진 과제 상태는 "이 블록 시작하기"만). `SplitPage`는 목적지로서 지원되나 진입 버튼이 없고, In-Scope·Positive Non-Goals·설계 결정 어디에도 이 엣지 컷이 명시되지 않았다(조용한 누락). 심각도 LOW — "쪼갠 것 중 만만한 1개 자기선택"(PH-05.1)이 이미 큐에서 조각을 고르게 해줘 재편집 필요가 약하고, One Task·상주목록 없음 정신상 재편집 재노출이 오히려 마찰일 수 있음. **처리: 의도적 컷 확정(2026-07-08 사용자 결정)** — SCREEN-FLOW §2의 `DASH → SPLIT(과제 재편집)` 실선은 MVP 컷. 재편집이 필요하면 현재 과제를 소진한 뒤 새로 쪼갬으로 갈음(별도 진입점 추가 안 함). 이 파일 완료 상태는 유지, 원 로직 재오픈 없음.
