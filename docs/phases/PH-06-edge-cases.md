...
- **(2026-07-08 SCREEN-FLOW 전이 소급 감사) MED 갭 1건, 미해결:** SPEC §6 "익일·장시간 방치 복귀 = 미완료 이월(자동완료 ❌)" · SCREEN-FLOW P4 "이월 블록은 다음날 그대로"의 **사용자 가시적 부활이 미실현**이다. (1) 큐(`queuedBlocks`)가 전부 인메모리라(PH-05 설계결정 1) 재부팅 후 큐가 비어 "다음날 그대로"가 애초에 살아남지 못한다 — 이 위상은 *재부팅 시 tasks/큐 복구*를 Positive Non-Goals·설계결정 3에서 명시적으로 컷했으나, **SPEC의 이월 '의미' 약속 자체를 "MVP 미달성(큐 영속 필요)"으로 어디에도 못박지 않았다**(조용한 누락). (2) 그와 별개로, 인세션 "오늘은 여기까지"(`RetroPage` 미완료 경로)가 이미 `dequeue`된 조각을 재큐잉하지 않아 **조각이 그 세션에서 영구 소멸**한다(살아있는 재개 경로는 "이어서 15분 더"뿐). K가 중간에 멈추면 그 조각을 잃는 경로라 SPEC §6 "이어하기·실패 무처벌·이월"과 어긋난다. code-reviewer(존재하는 코드의 결함)·수용 기준 자동화(존재하는 테스트 통과)·이 위상 당시 SPEC-only 커버리지 표 어느 것도 "SCREEN-FLOW/SPEC §6에만 있는 이월 의미 약속"을 잡지 못한 사각지대. **처리: 별도 수정 위상(가칭 PH-06.1) 분리 vs SPEC §6·SCREEN-FLOW P4를 'MVP 범위 밖(post-MVP 큐 영속)'으로 정합 — 아키텍처(Storage 인터페이스 확장 = 전역 DO NOT CHANGE)와 §2 무처벌·이월 철학이 걸린 제품 결정이라 사용자 결정 대기([README v1.4](README.md#changelog)).** 이 위상 완료 상태는 유지, 원 로직 재오픈 없음.
