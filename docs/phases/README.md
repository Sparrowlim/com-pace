...
| [PH-12](PH-12-fragment-wrapup.md)               | 조각 마무리 선택 (5-C, 자연 종료 시 완료/이어가기 사용자 결정)              | PH-05, PH-06                                   | **완료**(Runnable State 통과, 2026-07-14)                                                                                                                     |
...
- **v3.1(2026-07-18)** — **문서 정합성 감사 반영(코드 diff 없음).** §1 표의 `PH-12` 행이 실제로는 `PH-12-fragment-wrapup.md` v0.2(2026-07-14, 커밋 `3df37dc`)에서 구현·Runnable State 통과까지 끝났음에도(373/373 테스트, typecheck/lint/build 전부 exit 0) "계획 완료(상세화) — 구현 착수 대기"로 방치돼 있던 드리프트를 발견·정정("완료"로 갱신, `PH-04.4`/v2.6 사례와 동일 패턴). MVP 완료조건 점검 세션에서 사용자 요청으로 확인. 함께 확인한 항목: PH-12 이후 진행된 세션 완료 알람(탭 가시성별 차임/시스템 알림 분기)·모바일 input 확대 버그 수정·튜토리얼 없이도 되는 UX 개편(쪼개기 안내·집중 제스처 힌트)은 전부 `SPEC.md §4`(집중 화면 "소리/진동" 종료 신호)가 이미 확정한 범위 내 구현이라 신규 SSOT 드리프트 없음. 디자인 시스템 대개편(모듈 분해·화면 구성 규칙·앵커존 통일 등)은 `docs/design-system/CHANGELOG.md`가 v0.14까지 자체 추적 중이라 이 표에 별도 위상으로 등록하지 않음(design-system 문서군은 phases 위상 단위가 아니라 자체 버전으로 관리되는 별도 트랙).
