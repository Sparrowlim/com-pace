...
> 위 SSOT 발췌의 SPEC 문장/화살표 **및 SCREEN-FLOW 전이 행**을 전부 나열하고, 각각을 In-Scope 항목 / 착수 전 설계 결정 / Positive Non-Goals 중 정확히 하나에 배정한다. 어디에도 없는 문장/전이가 남으면 상세화 미완료. (SSOT = SPEC 문장 ∪ SCREEN-FLOW 전이 — SPEC 산문만 대조하면 상태기계 전이를 놓친다, README §0 ⚠️.)

| SPEC 문장/화살표                         | 매핑 위치                                      | 비고                |
| ---------------------------------------- | ---------------------------------------------- | ------------------- |
| (예: "쪼개진 것 중 만만한 1개 자기선택") | In-Scope B-3 / 설계 결정 4 / Positive Non-Goal | (단순화했다면 사유) |

## 전이 → 명명 테스트 (착수 직전 — [README §0](README.md#0-전역-규칙-모든-위상-공통--개별-ph-파일에서-반복-서술-금지) 전이→테스트 변환, RED 먼저)

> 위 SSOT 발췌의 각 전이 행·SPEC 문장마다 이음새 + 명명 테스트를 1행씩. **RED로 먼저 깔고 구현한다.** 이음새 = ① 순수 셀렉터/슬라이스(Vitest) · ② 라우트·상태 전이(Playwright) · ③ 불변식 어서션(Vitest+axe). "무엇을 테스트할지" 막막하면 대개 이음새 선택이 틀린 것. §0 북극성은 테스트 대상 아님(K 인터뷰).

| SSOT 전이/문장                        | 이음새 | 명명 테스트                                      |
| ------------------------------------- | ------ | ------------------------------------------------ |
| (예: 활성 과제 없이 `/focus` 진입)    | ②      | `redirects to dashboard when no active task`     |
| (예: 에너지 fill 완료/미완료 색 동일) | ③      | `energy fill color identical across done/undone` |
