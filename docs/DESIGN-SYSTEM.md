...
**남은 편차(다음 패스로 이월):** 두 그룹의 주 행동 버튼이 서로 다른 절대 위치(카드 그룹=상단 근처, 비카드 그룹=하단)에 있는 것 자체가 앱 전체 단일 기준으로 보면 편차다. 통일하려면 `TaskCard`를 하단 고정형으로 재구조화해야 하는데, 이는 온보딩·쪼개기·대시보드의 기존 검증된 인터랙션을 건드리는 더 큰 변경이라 이번 패스 범위 밖으로 명시적으로 미룬다. `DashboardPage`도 `AddTaskPrompt`(카드 없음) 상태와 `TaskCta` 등(카드 있음) 상태가 공존해 그룹이 섞여 있음 — 별도 패스 필요.

**편차로 남아 있는 것(별도 후속 필요):** 온보딩·zero 대시보드의 `TaskCard` 경계가 배경(`surface-page`)과 명도차가 작아 흐릿하게 보이는 편차는 이번 패스에서 손대지 않았다 — primitive 색 값 변경은 `DESIGN-TOKENS.md` 소유라 범위가 다르다.
