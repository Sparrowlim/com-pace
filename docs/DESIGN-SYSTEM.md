...
**✔ 해소(v0.9, 2026-07-13):** `NorthStarPage`의 "남길게요"와 `SettingsPage`의 "양가 목표 수정"이 `variant="primary"`(테라코타 글로우)를 쓰고 있어, 타이머 시작 같은 "즉시성의 순간" 전용 CTA와 동일한 시각적 무게를 갖던 위반을 발견·수정 — 둘 다 `variant="secondary"`로 낮췄다. 근거: CLAUDE §5(북극성은 "가벼운 좌표"·선택 사항·순위 강요 금지)와 위 표의 "즉시성의 순간에만" 원칙이 정면 충돌하고 있었다. `NorthStarPage`의 남기기/건너뛰기도 이제 동일한 보조 무게라 어느 쪽으로도 시각적으로 떠밀지 않는다.
...
| `NorthStarBadge` | `src/components/NorthStarBadge`                                            | 열망·의무를 독립된 칩 2개로 나란히 렌더(v0.10 — 합쳐진 한 줄 텍스트에서 전환). `Chip.default`와 같은 톤(`chip-bg`/`chip-line`)이지만 `<span>`이라 `onClick` prop이 타입에 존재하지 않음(탭 핸들러 자체가 불가능) | SPEC §9 "두 좌표 나란히, 순위 없음"(D-19) · CLAUDE §1·§5                   |
...
- **v0.10(2026-07-13)** — **북극성 "표현하는 형식" 자체를 SPEC 정본대로 재구성(§5 `NorthStarBadge`).** v0.9는 버튼(CTA) 무게를 다뤘지만, 사용자는 이번엔 배지의 형식 자체를 가리켰다 — `SPEC.md` §9("의무·열망 두 좌표 나란히, 순위 없음"(D-19), "대시보드 두 칩과 정합")와 실제 구현을 대조하니, `NorthStarBadge`가 `formatNorthStarSummary`로 "열망: X · 의무: Y"를 한 줄 텍스트에 합쳐 렌더하고 있어 SPEC이 명시한 "두 칩"이 아니었다. `NorthStarBadge`를 `Chip.default`와 같은 톤(`chip-bg`/`chip-line`, 그림자 없음)의 독립된 `<span>` 2개(열망·의무 각자)로 재구성 — 클릭 불가 시그니처(PH-04.4 1-5)는 유지, `formatNorthStarSummary`는 `role="group"`의 `aria-label`로 재사용(스크린리더에 한 번에 읽히도록, `EnergyBar` 캡션 선례와 동일 결). 부수 효과로 `DashboardHeader`가 320px에서 칩 2개+"설정" 버튼을 한 줄에 욱여넣다 "설정" 텍스트가 깨지던(줄바꿈) 문제를 발견 — `.header`에 `flex-wrap`을 추가해 칩 그룹이 필요하면 자기 줄로 내려가고 "설정"은 항상 온전한 크기를 유지하도록 수정. 유닛 340개(+2 신규)·e2e 116개 전부 그린, `core-loop-catalog`·`zero-dashboard` 스냅샷은 헤더 여백 미세 변화로 재생성(북극성 미설정 상태라 칩 자체는 등장하지 않음).
- **v0.9(2026-07-13)** — **북극성 시각적 무게 재조정(§3 elevation).** 사용자가 "북극성이 도드라져 보인다"고 지적, 원인은 `NorthStarPage`(남길게요)·`SettingsPage`(양가 목표 수정)가 `variant="primary"`(테라코타 `elevation.cta` 글로우)를 쓰고 있던 것 — 타이머 시작·블록 확정 같은 "즉시성의 순간" 전용 CTA와 시각적으로 구분이 안 됐다. CLAUDE §5(북극성 = 진행 측정기 아닌 가벼운 좌표, 선택 사항, 단수·순위 강요 금지)와 정면 충돌하는 기존 위반이라 `variant="secondary"`로 낮춰 해소. 남기기/건너뛰기 두 버튼이 이제 동일한 보조 무게라 어느 쪽으로도 떠밀지 않는다. `NorthStarBadge`(순수 텍스트, 무-elevation)는 이미 적절했으므로 무변경. 새 색·새 컴포넌트 없음, 기존 `Button.secondary` 변형 재사용.
