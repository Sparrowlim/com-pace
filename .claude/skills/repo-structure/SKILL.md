...
전체 파일 목록은 **손으로 유지하지 않는다** — `.claude/hooks/update-repo-map.cjs`가 `src/`·`docs/` 아래 Write 시마다 `GENERATED.md`를 재생성한다. 이 파일은 목적/컨벤션만 담당하고 재기술하지 않는다(DRY).
...
→ `.claude/skills/repo-structure/GENERATED.md` (자동 생성, Write 훅이 항상 최신 유지)
